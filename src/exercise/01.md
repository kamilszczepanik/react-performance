...
### When to use this technics

- When I want to create react native application where interaction is a priority
  and I want to avoid unecessary re-renders.
- Rendering huge data
- React has tools to use to speed up the application. A lot of the time the code
  is the same except the mini part where this is used.

#### Mobile browser is slower than laptop browser

You can use Components in web browser to check what will suspense for the
specific component look like. This is the STOPER ICON next to component name.

##### Usage Visualization - contains dependencies that are run and not run(red) and lines that are not run. To optimize performance.

### Coverage

Coverage in dev tools, loading less code and running less code is better. There
is a tab that you can check this. Left bottom - how many bytes we've loaded.
