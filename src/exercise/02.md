...
Performance tab has ability to see what is loading and for how long. It is good
to select 6x slower cpu and record interactions with browser. After this you
have the stats to see if something is slow and takes much time.

React is much faster in production! You should check before improving
performance how the app will behave in production.

You can take some js that is expensive to run and put it on the other thread so
the main thread is freed up to display things to the user more rapidly. Thanks
to for example workerize loader in webpack. This is async.
