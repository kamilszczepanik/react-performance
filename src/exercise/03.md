...
### It is important that function has a name, because the function then will pop up in the dev tools!

Using react profiler helps with checking out the app speed which is pretty
important when it comes to user experience. Everything we needed to do if we've
found that there is a place and need for improvement was
`Component = React.memo(Component)`

- Highlighting one item should only re-render this item, not also the other
  items!
- If you have a particular component and you're rendering tons of instances of
  those, try to do calculations a little higher in a tree, so you only passing
  primitive values that when change will trigger a DOM update. That way you
  don't worry about breaking memoization or having to create custom comperator.
